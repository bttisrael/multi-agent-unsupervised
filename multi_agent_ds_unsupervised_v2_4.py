"""
Auto Segmenter AI v2.4 — Multi-Agent Unsupervised Learning + GitOps Pipeline
=================================================================
...
def prepare_cluster_matrix(features: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    # v2.3 fix:
    # Some datasets do not contain product or quantity information. In those cases,
    # features such as total_quantity, avg_quantity, product_diversity and
    # revenue_per_product can be 100% null. SimpleImputer drops all-null columns,
    # which creates a mismatch between the transformed array and feature_cols.
    # We remove all-null and near-constant columns before the sklearn pipeline.
    all_null_cols = [c for c in X.columns if X[c].notna().sum() == 0]
    if all_null_cols:
        logger.info("[Clustering] Dropping all-null numeric features: %s", all_null_cols)
    ...
    if constant_cols:
        logger.info("[Clustering] Dropping constant numeric features: %s", constant_cols)
        X = X.drop(columns=constant_cols)

    feature_cols = X.columns.tolist()
    if not feature_cols:
        raise RuntimeError("No usable numeric features available after dropping all-null/constant columns.")
    ...
    steps = [("imputer", SimpleImputer(strategy="median", keep_empty_features=False))]
    ...
    # Extra guard: if sklearn ever drops columns, align names to the resulting width.
    if arr.shape[1] != len(feature_cols):
        logger.warning(
            "[Clustering] Feature width changed during preprocessing: arr=%s, feature_cols=%s. "
            "Aligning names to transformed matrix width.",
            arr.shape[1], len(feature_cols)
        )
        feature_cols = feature_cols[:arr.shape[1]]
...
def generate_deliverables() -> str:
    try:
        ...
        return (
            ...
            f"html={out_path('html_dashboard')}, app={out_path('app')}, "
            f"notebook={out_path('notebook')}, readme={out_path('readme')}"
        )
    except Exception as exc:
...
def generate_html_dashboard() -> None:
    """Generate a static, portfolio-ready HTML dashboard that works without Streamlit."""
    import html
    ...
    hypotheses = load_json(out_path("hypotheses_json")) if out_path("hypotheses_json").exists() else []
    cfg = load_json(out_path("entity_config")) if out_path("entity_config").exists() else {}
    ...
    try:
        if out_path("silver").exists():
            raw_rows = f"{pd.read_parquet(out_path('silver')).shape[0]:,}"
        if out_path("clustered").exists():
            cdf = pd.read_parquet(out_path("clustered"))
            entity_rows = f"{len(cdf):,}"
            feature_count = f"{len([c for c in cdf.columns if c != 'cluster']):,}"
            if "total_revenue" in cdf.columns:
                total_revenue = float(cdf["total_revenue"].sum())
    except Exception:
        pass

    def img_tag(filename: str, title: str) -> str:
        p = CONFIG.out / filename
        if not p.exists():
            return ""
        return f"""
    ...
    segment_cards = []
    for seg in sorted(profiles.get("segments", []), key=lambda x: x.get("cluster", 0)):
        high = ", ".join(seg.get("drivers_high", [])) or "N/A"
        low = ", ".join(seg.get("drivers_low", [])) or "N/A"
        segment_cards.append(f"""
        <div class="segment-card">
            <div class="segment-header">
                <span class="cluster-pill">Cluster {seg.get('cluster')}</span>
                <h3>{html.escape(str(seg.get('segment_name', 'Segment')))}</h3>
            </div>
            <p class="segment-size">{seg.get('size', 0):,} entities · {seg.get('size_pct', 0):.1%}</p>
            <p><strong>Persona:</strong> {html.escape(str(seg.get('persona', 'Behavior-based customer segment.')))}</p>
            <p><strong>High drivers:</strong> {html.escape(high)}</p>
            <p><strong>Low drivers:</strong> {html.escape(low)}</p>
            <p><strong>Recommended action:</strong> {html.escape(str(seg.get('recommended_action', 'N/A')))}</p>
            <p><strong>Opportunity:</strong> {html.escape(str(seg.get('business_opportunity', 'N/A')))}</p>
            <p><strong>Risk:</strong> {html.escape(str(seg.get('business_risk', 'N/A')))}</p>
        </div>
        """)
    ...
    for h in hypotheses[:12]:
        ...
        cls = "true" if verdict == "TRUE" else "false" if verdict == "FALSE" else "neutral"
        hyp_rows.append(f"""
        <tr>
            <td>{html.escape(str(h.get('feature', '')))}</td>
            <td>{html.escape(str(h.get('test', '')))}</td>
            <td><span class="verdict {cls}">{html.escape(str(verdict))}</span></td>
            <td>{h.get('p_value', 0):.4g}</td>
            <td>{html.escape(str(h.get('business_insight', '')))}</td>
        </tr>
        """)

    total_revenue_card = (
        f"""<div class="metric-card"><span>Total Revenue</span><strong>{total_revenue:,.0f}</strong></div>"""
        if total_revenue is not None else ""
    )

    best_json = html.escape(json.dumps(best, indent=2, ensure_ascii=False, default=json_safe))
    cfg_json = html.escape(json.dumps(cfg, indent=2, ensure_ascii=False, default=json_safe))

    dashboard = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Auto Segmenter AI — HTML Dashboard</title>
...
<style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .metric-card, .card, .segment-card, .chart-card {{
        border: 1px solid var(--border);
    ...
    .true {{ background: rgba(34,197,94,0.15); color: #86efac; }}
    .false {{ background: rgba(239,68,68,0.15); color: #fca5a5; }}
    .neutral {{ background: rgba(245,158,11,0.15); color: #fcd34d; }}
    .footer {{
        margin-top: 48px;
        color: var(--muted);
        text-align: center;
...
<div class="container">
    <section class="hero">
        <span class="tag">Unsupervised Learning · Customer Segmentation · RGM</span>
        <h1>Auto Segmenter AI</h1>
        <p>
            A multi-agent unsupervised learning pipeline that transforms transactional data into
            customer-level intelligence, business segment profiles and revenue growth actions.
        </p>
    </section>

    <section class="grid">
        <div class="metric-card"><span>Raw Rows</span><strong>{raw_rows}</strong></div>
        <div class="metric-card"><span>Clustered Entities</span><strong>{entity_rows}</strong></div>
        <div class="metric-card"><span>Features</span><strong>{feature_count}</strong></div>
        <div class="metric-card"><span>Best Model</span><strong>{html.escape(str(best.get('model_name', 'N/A')))}</strong></div>
        {total_revenue_card}
    </section>

    <section class="grid">
        <div class="metric-card"><span>Clusters</span><strong>{html.escape(str(best.get('n_clusters', 'N/A')))}</strong></div>
        <div class="metric-card"><span>Silhouette</span><strong>{html.escape(str(best.get('silhouette', 'N/A')))}</strong></div>
        <div class="metric-card"><span>Davies-Bouldin</span><strong>{html.escape(str(best.get('davies_bouldin', 'N/A')))}</strong></div>
        <div class="metric-card"><span>Calinski-Harabasz</span><strong>{html.escape(str(best.get('calinski_harabasz', 'N/A')))}</strong></div>
    </section>

    <h2>Executive Summary</h2>
    <div class="card">
        <p>
            This project goes beyond a simple KMeans notebook. It builds an end-to-end segmentation
            product: ingestion, entity detection, RFM feature engineering, clustering model competition,
            statistical validation, segment interpretation, business recommendations, app generation and GitOps.
        </p>
        <p>
            The result is a reusable customer intelligence system that can be rerun when the dataset changes.
        </p>
    </div>

    <h2>Visual Diagnostics</h2>
    <div class="charts">
        {img_tag(ARTIFACTS["pca_png"], "Cluster Map — PCA Projection")}
        {img_tag(ARTIFACTS["rfm_png"], "RFM Segment Profile")}
        {img_tag(ARTIFACTS["sizes_png"], "Cluster Size Distribution")}
        {img_tag(ARTIFACTS["heatmap_png"], "Cluster Feature Heatmap")}
    </div>
    ...
    <h2>Detected Entity Configuration</h2>
    <pre>{cfg_json}</pre>
    ...
    <div class="card">
        <p>
            This static dashboard can be opened directly in any browser or published with GitHub Pages.
            For deeper interaction, the pipeline also generates a Streamlit app:
        </p>
        <pre>streamlit run unsupervised_outputs/streamlit_segment_app.py</pre>
    </div>

    <div class="footer">
        Generated automatically by Auto Segmenter AI v2.4.
    </div>
</div>
</body>
</html>
"""
    out_path("html_dashboard").write_text(dashboard, encoding="utf-8")
...
def generate_notebook() -> None:
    """
    Generate a detailed technical notebook.

    Earlier versions generated a thin summary notebook. This version creates a
    developer-friendly notebook that documents the full pipeline logic, loads all
    artifacts, displays metrics/charts, and includes the full source code appendix.
    """
    try:
        import nbformat
    ...
    current_script = Path(sys.argv[0]).name if sys.argv and sys.argv[0] else "multi_agent_ds_unsupervised_v2_4.py"
    script_text = Path(current_script).read_text(encoding="utf-8") if Path(current_script).exists() else ""
...
This notebook is automatically generated by **Auto Segmenter AI v2.4**.

It is designed to be a detailed technical companion to the portfolio project. It includes:

1. Pipeline configuration
2. Data ingestion outputs
3. Entity detection and data profiling
4. Feature engineering analysis
5. Clustering matrix preparation
6. Model competition and best model selection
7. Segment interpretation
8. Statistical hypothesis validation
9. Visual diagnostics
10. Full pipeline source code appendix
"""))
...
BASE = Path(".")
print("Current directory:", BASE.resolve())
"""))

    cells.append(new_markdown_cell("## 1. Pipeline Configuration"))
    cells.append(new_code_cell(f"""CONFIG = {json.dumps(asdict(CONFIG), indent=2, ensure_ascii=False, default=json_safe)}
ARTIFACTS = {json.dumps(ARTIFACTS, indent=2, ensure_ascii=False)}
print(json.dumps(CONFIG, indent=2, ensure_ascii=False))
"""))

    cells.append(new_markdown_cell("## 2. Pipeline Run Results"))
    cells.append(new_code_cell("""run_results_path = BASE / "pipeline_run_results.json"
...
"""))
    ...
    cells.append(new_markdown_cell("## 3. Silver Dataset"))
    cells.append(new_code_cell(f"""silver_path = BASE / "{ARTIFACTS['silver']}"
if silver_path.exists():
    ...
    print("Silver file not found:", silver_path)
"""))

    cells.append(new_markdown_cell("## 4. Data Profile and Entity Detection"))
    cells.append(new_code_cell(f"""profile_md = BASE / "{ARTIFACTS['profile']}"
entity_config_path = BASE / "{ARTIFACTS['entity_config']}"

if entity_config_path.exists():
    entity_config = json.loads(entity_config_path.read_text(encoding="utf-8"))
    print(json.dumps(entity_config, indent=2, ensure_ascii=False))
else:
    print("entity_config.json not found.")

if profile_md.exists():
    display(Markdown(profile_md.read_text(encoding="utf-8")))
else:
    print("Data_Profile.md not found.")
"""))

    cells.append(new_markdown_cell("## 5. Feature Engineering Output"))
    cells.append(new_code_cell(f"""features_path = BASE / "{ARTIFACTS['features']}"
feature_report_path = BASE / "{ARTIFACTS['feature_report']}"

if features_path.exists():
    features = pd.read_parquet(features_path)
    print("Feature table shape:", features.shape)
    display(features.head())
    display(features.describe(include="all").T.head(40))
else:
    print("Feature table not found.")

if feature_report_path.exists():
    display(Markdown(feature_report_path.read_text(encoding="utf-8")))
else:
    print("Feature_Engineering_Report.md not found.")
"""))

    cells.append(new_markdown_cell("## 6. Clustering Matrix"))
    cells.append(new_code_cell(f"""matrix_path = BASE / "{ARTIFACTS['cluster_matrix']}"
if matrix_path.exists():
    cluster_matrix = pd.read_parquet(matrix_path)
    print("Cluster matrix shape:", cluster_matrix.shape)
    display(cluster_matrix.head())
    display(cluster_matrix.describe().T.head(40))
else:
    print("Cluster matrix not found.")
"""))

    cells.append(new_markdown_cell("## 7. Clustering Model Competition"))
    cells.append(new_code_cell(f"""metrics_path = BASE / "{ARTIFACTS['metrics']}"
if metrics_path.exists():
    metrics = json.loads(metrics_path.read_text(encoding="utf-8"))
    print("Best model:")
    print(json.dumps(metrics.get("best_model", {{}}), indent=2, ensure_ascii=False))

    all_models = pd.DataFrame(metrics.get("all_models", []))
    if not all_models.empty:
        cols = [c for c in [
            "model_name", "n_clusters", "silhouette", "davies_bouldin",
            "calinski_harabasz", "min_cluster_pct", "business_score", "is_valid"
        ] if c in all_models.columns]
        display(all_models[cols].head(30))
else:
    print("cluster_metrics.json not found.")
"""))

    cells.append(new_markdown_cell("## 8. Final Clustered Customers / Entities"))
    cells.append(new_code_cell(f"""clustered_path = BASE / "{ARTIFACTS['clustered']}"
if clustered_path.exists():
    clustered = pd.read_parquet(clustered_path)
    print("Clustered table shape:", clustered.shape)
    display(clustered.head())
    display(clustered["cluster"].value_counts(normalize=True).sort_index().to_frame("share"))

    numeric_cols = [c for c in clustered.select_dtypes(include="number").columns if c != "cluster"]
    display(clustered.groupby("cluster")[numeric_cols].median().T.head(60))
else:
    print("Clustered customers file not found.")
"""))

    cells.append(new_markdown_cell("## 9. Segment Profiles and Business Strategy"))
    cells.append(new_code_cell(f"""segment_md = BASE / "{ARTIFACTS['segment_md']}"
strategy_md = BASE / "{ARTIFACTS['strategy_md']}"
profiles_json = BASE / "{ARTIFACTS['segment_profiles']}"

if segment_md.exists():
    display(Markdown(segment_md.read_text(encoding="utf-8")))
else:
    print("Segment_Profiles.md not found.")

if strategy_md.exists():
    display(Markdown(strategy_md.read_text(encoding="utf-8")))
else:
    print("Business_Strategy.md not found.")

if profiles_json.exists():
    profiles = json.loads(profiles_json.read_text(encoding="utf-8"))
    print("Segment profile JSON keys:", profiles.keys())
"""))

    cells.append(new_markdown_cell("## 10. Hypothesis Validation"))
    cells.append(new_code_cell(f"""hyp_md = BASE / "{ARTIFACTS['hypotheses_md']}"
hyp_json = BASE / "{ARTIFACTS['hypotheses_json']}"

if hyp_json.exists():
    hyp = pd.DataFrame(json.loads(hyp_json.read_text(encoding="utf-8")))
    display(hyp)
else:
    print("segment_hypotheses.json not found.")

if hyp_md.exists():
    display(Markdown(hyp_md.read_text(encoding="utf-8")))
else:
    print("Segment_Hypothesis_Validation.md not found.")
"""))

    cells.append(new_markdown_cell("## 11. Visual Diagnostics"))
    cells.append(new_code_cell(f"""image_files = [
    "{ARTIFACTS['pca_png']}",
    "{ARTIFACTS['sizes_png']}",
    "{ARTIFACTS['rfm_png']}",
    "{ARTIFACTS['heatmap_png']}",
]
...
"""))
    ...
    cells.append(new_markdown_cell("## 12. Static HTML Dashboard"))
    cells.append(new_code_cell(f"""dashboard_path = BASE / "{ARTIFACTS.get('html_dashboard', 'dashboard.html')}"
if dashboard_path.exists():
    print("Static dashboard generated at:", dashboard_path.resolve())
else:
    print("dashboard.html not found.")
"""))

    cells.append(new_markdown_cell("""## 13. How the Pipeline Works

The pipeline is organized as function-based agents:

| Agent | Role |
|---|---|
| Ingestor | Loads Kaggle/local dataset and saves the silver layer |
| Data Profiler | Detects entity, date, monetary, order and product columns |
| Feature Engineer | Builds entity-level RFM and behavioral features |
| Clustering Scientist | Preprocesses features and compares clustering algorithms |
| Segment Interpreter | Converts clusters into business-readable segment profiles |
| Hypothesis Validator | Tests whether clusters differ statistically |
| Deliverable Generator | Creates app, dashboard, README and notebook |
| GitOps Agent | Commits and pushes portfolio artifacts |

The heavy computation is deterministic Python. Optional LLM calls are used for interpretation, not for core clustering correctness.
"""))

    cells.append(new_markdown_cell("## 14. Full Source Code Appendix"))
    if script_text:
        ...
        for section in sections:
            ...
            first_line = section.splitlines()[0][:120]
            if "# Agent" in section[:300]:
                title = first_line.replace("#", "").strip()
            elif "# Configuration" in section[:300]:
                title = "Configuration"
            elif "# Utilities" in section[:300]:
                title = "Utilities"
            elif "# Orchestration" in section[:300]:
                title = "Orchestration"
            cells.append(new_markdown_cell(f"### {title}"))
            cells.append(new_code_cell(section))
    else:
        cells.append(new_code_cell("# Source script was not found in the current folder when notebook was generated."))
...
def run_pipeline() -> None:
    ensure_output_dir()
    logger.info("Starting Auto Segmenter AI v2.4.1")
    ...
    logger.info("Open static dashboard: %s", out_path("html_dashboard"))
    logger.info("Open Streamlit app with: streamlit run %s", out_path("app"))
